...
class Node(VGroup):
    ...
    def __init__(self, val, **kwargs):
        ...
        text = Tex(str(val), color = BLUE)
        line = Line()
        node = self.add(circle, text, line)
    ...
    def returnTree(self, node):
        ...
        if node is not None:
            ...
            if left is not None:
                ...
                left.pos = left_pos
                #left.lne = Line(node.get_center() + (DL * (node.rad / math.sqrt(2))), left_pos  + (UR * (node.rad/ math.sqrt(2))))
                left.lne = Line(node.get_center(), left_pos)
            ...
            if right is not None:
                ...
                right.pos = right_pos
                #right.lne = Line(node.get_center() + (DR * (node.rad/ math.sqrt(2))), right_pos + (UL * (node.rad/ math.sqrt(2))))
                right.lne = Line(node.get_center(), right_pos)
...
class RedBlackTree(Tree):
    ...
    def setColor(self, node):
        if node.color == 1:
            node.set_fill(color = NEW_RED)
            node.set_stroke(color = WHITE)
            (node.txt).set_fill(color = WHITE)
        else:
            node.set_fill(color = BLACK)
            node.set_stroke(color = WHITE)
            (node.txt).set_fill(color = WHITE)
        return node
